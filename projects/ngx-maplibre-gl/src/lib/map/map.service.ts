import { Injectable, OutputEmitterRef, signal } from "@angular/core";
import {
  type CameraOptions,
  type FlyToOptions,
  type LngLatLike,
...
export class MapService {
  ...
  addLayer(layer: SetupLayer, bindEvents: boolean, before?: string) {
    ...
    this.mapInstance.addLayer(layer.layerOptions as LayerSpecification, before);
    if (!bindEvents) {
      return;
    }
  ...
  private hookEvents(events: MapEvent) {
    ...
    this.mapInstance.on("resize", (evt) => events.mapResize.emit(evt));
    this.mapInstance.on("remove", (evt) => events.mapRemove.emit(evt));
    this.mapInstance.on("mousedown", (evt) => events.mapMouseDown.emit(evt));
    this.mapInstance.on("mouseup", (evt) => events.mapMouseUp.emit(evt));
    this.mapInstance.on("mousemove", (evt) => events.mapMouseMove.emit(evt));
    this.mapInstance.on("click", (evt) => events.mapClick.emit(evt));
    this.mapInstance.on("dblclick", (evt) => events.mapDblClick.emit(evt));
    this.mapInstance.on("mouseover", (evt) => events.mapMouseOver.emit(evt));
    this.mapInstance.on("mouseout", (evt) => events.mapMouseOut.emit(evt));
    this.mapInstance.on("contextmenu", (evt) =>
      events.mapContextMenu.emit(evt)
    );
    this.mapInstance.on("touchstart", (evt) => events.mapTouchStart.emit(evt));
    this.mapInstance.on("touchend", (evt) => events.mapTouchEnd.emit(evt));
    this.mapInstance.on("touchmove", (evt) => events.mapTouchMove.emit(evt));
    this.mapInstance.on("touchcancel", (evt) =>
      events.mapTouchCancel.emit(evt)
    );
    this.mapInstance.on("wheel", (evt) => events.mapWheel.emit(evt));
    this.mapInstance.on("movestart", (evt) => events.moveStart.emit(evt));
    this.mapInstance.on("move", (evt) => events.move.emit(evt));
    this.mapInstance.on("moveend", (evt) => events.moveEnd.emit(evt));
    this.mapInstance.on("dragstart", (evt) => events.mapDragStart.emit(evt));
    this.mapInstance.on("drag", (evt) => events.mapDrag.emit(evt));
    this.mapInstance.on("dragend", (evt) => events.mapDragEnd.emit(evt));
    this.mapInstance.on("zoomstart", (evt) => events.zoomStart.emit(evt));
    this.mapInstance.on("zoom", (evt) => events.zoomEvt.emit(evt));
    this.mapInstance.on("zoomend", (evt) => events.zoomEnd.emit(evt));
    this.mapInstance.on("rotatestart", (evt) => events.rotateStart.emit(evt));
    this.mapInstance.on("rotate", (evt) => events.rotate.emit(evt));
    this.mapInstance.on("rotateend", (evt) => events.rotateEnd.emit(evt));
    this.mapInstance.on("pitchstart", (evt) => events.pitchStart.emit(evt));
    this.mapInstance.on("pitch", (evt) => events.pitchEvt.emit(evt));
    this.mapInstance.on("pitchend", (evt) => events.pitchEnd.emit(evt));
    this.mapInstance.on("boxzoomstart", (evt) => events.boxZoomStart.emit(evt));
    this.mapInstance.on("boxzoomend", (evt) => events.boxZoomEnd.emit(evt));
    this.mapInstance.on("boxzoomcancel", (evt) =>
      events.boxZoomCancel.emit(evt)
    );
    this.mapInstance.on("webglcontextlost", (evt) =>
      events.webGlContextLost.emit(evt)
    );
    this.mapInstance.on("webglcontextrestored", (evt) =>
      events.webGlContextRestored.emit(evt)
    );
    this.mapInstance.on("render", (evt) => events.render.emit(evt));
    this.mapInstance.on("error", (evt) => events.mapError.emit(evt));
    this.mapInstance.on("data", (evt) => events.data.emit(evt));
    this.mapInstance.on("styledata", (evt) => events.styleData.emit(evt));
    this.mapInstance.on("sourcedata", (evt) => events.sourceData.emit(evt));
    this.mapInstance.on("dataloading", (evt) => events.dataLoading.emit(evt));
    ...
    this.mapInstance.on("idle", (evt) => events.idle.emit(evt));
  }
